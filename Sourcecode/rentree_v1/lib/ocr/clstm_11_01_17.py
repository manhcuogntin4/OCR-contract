...
def clstm_ocr_parallel_new(img, islieu=False):
	if not os.path.exists(CACHE_FOLDER):
		os.makedirs(CACHE_FOLDER)
	model_path = os.path.join(this_dir, 'model-nomprenom2911-binary.clstm')
	if islieu:
	...
	txt={}
	prob={}
	cropY_H=cropY*cropHeight
	for j in range (0,cropY_H):
		q[j] = multiprocessing.Queue()
		p[j] = multiprocessing.Process(target=calib_clstm_x_width_queue, args=(cropX,j%8,cropWidth,j/8,image,model_path,q[j]))
		p[j].start()
	for j in range (0,cropY_H):
		#p[j].join()
		txt[j],prob[j]=q[j].get()
...
def calib_clstm_x_width(cropX, y, cropWidth, h, image, model_path):
	maxPro=0
	ocr_result=""
...
def calib_clstm_x_width_queue(cropX, y, cropWidth, h, image, model_path, q):
	q.put(calib_clstm_height_low(cropX, y, cropWidth, h, image, model_path))
