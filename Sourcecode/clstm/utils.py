...
def copy_file_folder(filename, folder):
	print "folder dir", folder
	basename=os.path.basename(filename)
	print basename
	filename_new=os.path.join(folder, basename)
...
def get_numero_filename(filename):
	index=filename.find('.')
...
if __name__ == '__main__':
	...
	'''
	#filename="20.bin.png"
	#print get_numero_filename(filename)

	# ls_file=read_file_folder("./", "*.png")
	# GPS_FOLDER="GPS3_good"
	# create_folder(GPS_FOLDER)
	# print ls_file
	# for file in ls_file:
	# 	txt=read_tesseract_file(file)

	# 	id_file=get_numero_filename(get_base_name(file))
	# 	file_txt=str(id_file)+".gt.txt"
	# 	if check_file_exist(file_txt):
	# 		f=open(file_txt, "r")
	# 		t=f.read()
	# 		print txt, t
	# 		if(txt==t) and txt!="":
	# 			print file
	# 			copy_file_folder(file, GPS_FOLDER)
	# 			copy_file_folder(file_txt,GPS_FOLDER)
	# 		else:
	# 			regex=r"\b[0-9]+/[0-9]+\b"
	# 			txt=extract_info(txt, regex)
	# 			if txt!="":
	# 				f=open(file_txt,"w")
	# 				f.write(txt)
	# 				copy_file_folder(file, GPS_FOLDER)
	...
	ls_file=read_file_folder("./GPS3_good/", "*.txt")
	for file in ls_file:
		f=open(file, "r")
		txt=f.read()
		if txt.strip()=="":
